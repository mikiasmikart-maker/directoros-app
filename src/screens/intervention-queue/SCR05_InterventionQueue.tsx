      ...
      <div className="grid h-full min-h-0 grid-rows-[auto_1fr_auto] gap-3">
        <section className="m6-tier-2 rounded-md px-4 py-3">
          <div className="grid grid-cols-[repeat(6,minmax(0,1fr))] gap-2 text-[10px] uppercase tracking-wide">
            <div className="rounded bg-dos-panel/30 px-2 py-1.5 text-dos-text-muted/60">Queue Health: <span className="text-dos-text/90">{queueHealthLabel}</span></div>
            <div className="rounded bg-dos-sig-warning/8 px-2 py-1.5 text-dos-sig-warning/90">Urgent: {urgentCount}</div>
            <div className="rounded bg-dos-sig-drift/8 px-2 py-1.5 text-dos-sig-drift/90">Aging: {agingCount}</div>
            <div className="rounded bg-dos-panel/30 px-2 py-1.5 text-dos-text-muted/60">Blocked: <span className="text-dos-text/90">{blockedCount}</span></div>
            <div className="rounded bg-dos-sig-continuity/8 px-2 py-1.5 text-dos-sig-continuity/90">SLA: {slaCounterLabel}</div>
            <div className="rounded bg-dos-sig-continuity/8 px-2 py-1.5 text-dos-sig-continuity/90">path: {quickPathLabel ?? 'create → assign → resolve'}</div>
          </div>
        </section>
          ...
          <section className="m6-tier-2 min-h-0 rounded-md p-3">
            ...
            <div className="max-h-[58vh] space-y-1.5 overflow-auto pr-0.5">
              {interventions.length ? (
                interventions.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full rounded border px-3 py-2 text-left transition-all ${isSelected ? 'border-dos-sig-continuity/28 bg-dos-sig-continuity/10 shadow-[0_3px_10px_rgba(207,140,255,0.08)]' : 'border-white/[0.025] bg-dos-panel/30 hover:border-white/[0.035] hover:bg-dos-panel/36 hover:text-dos-text/90'}`}
                    >
                ...
                <div className="rounded bg-dos-panel/30 px-2 py-2 text-[11px] text-dos-text-muted/80">No interventions queued.</div>
              )}
            </div>
          </section>
          ...
          <aside className="m6-tier-1 min-h-0 rounded-md p-4">
            ...
            {selected ? (
              <div className="space-y-2 text-[11px]">
                <div className="rounded bg-dos-panel/30 px-2 py-1.5 text-dos-text/90">{selected.title}</div>
                ...
                <section className="rounded bg-dos-panel/30 p-2">
                ...
                <section className="rounded bg-dos-panel/32 p-3">
                ...
                <div className="flex flex-wrap gap-1.5 text-[10px] uppercase tracking-wide">
                  <button type="button" onClick={() => onAssign?.(selected.id)} className="rounded border border-dos-border px-2 py-1 text-dos-text-muted/80 hover:bg-dos-panel/34 hover:text-dos-text/90">Assign</button>
                  <button type="button" onClick={() => onDefer?.(selected.id)} className="rounded border border-dos-border px-2 py-1 text-dos-text-muted/80 hover:bg-dos-panel/34 hover:text-dos-text/90">Defer</button>
              ...
              <div className="rounded bg-dos-panel/30 px-2 py-2 text-[11px] text-dos-text-muted/80">Select an intervention to inspect details.</div>
            )}
          </aside>
        </section>

        <section className="rounded-md bg-dos-panel/28 px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.008)]">
          <div className="flex items-center justify-between gap-6">
            <div className="flex items-center gap-1.5">
              <button 
                ...
                className="rounded-md bg-dos-sig-continuity px-6 py-2 text-[11px] font-bold uppercase tracking-[0.12em] text-white shadow-md shadow-dos-sig-continuity/10 transition-colors hover:brightness-105 active:scale-[0.99] disabled:opacity-40"
              >
                Resolve
              </button>
              <div className="h-8 w-px bg-white/[0.025] mx-2" />
            ...
            <div className="flex items-center gap-2">
              <button type="button" disabled={!selected} onClick={() => selected && onSupersede?.(selected.id)} className="rounded border border-dos-border px-3 py-1.5 text-[10px] uppercase tracking-wide text-dos-text-muted/80 hover:bg-dos-panel/34 hover:text-dos-text disabled:opacity-40">Close</button>
              <button type="button" onClick={onCreateQuickIntervention} className="rounded border border-dos-sig-continuity/30 bg-dos-sig-continuity/10 px-3 py-1.5 text-[10px] uppercase tracking-wide text-dos-sig-continuity/90 hover:bg-dos-sig-continuity/14">Manual Override</button>
            </div>
          </div>
        </section>
      </div>
