      ...
      <div className="grid h-full min-h-0 grid-rows-[auto_1fr_auto] gap-3">
        <section className="m6-tier-2 rounded-md px-4 py-3 border border-white/5">
          <div className="grid grid-cols-[repeat(6,minmax(0,1fr))] gap-2 text-[10px] uppercase tracking-wide">
            ...
            <div className="rounded border border-dos-border bg-dos-panel/45 px-2 py-1.5 text-textMuted">Blocked: <span className="text-text/92">{blockedCount}</span></div>
                ...
                interventions.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full rounded border px-3 py-2 text-left transition-all ${isSelected ? 'border-dos-sig-continuity/40 bg-dos-sig-continuity/12 shadow-[0_4px_16px_rgba(207,140,255,0.12)]' : 'border-dos-border bg-dos-panel/45 hover:border-dos-border hover:bg-dos-panel/52 hover:text-text/88'}`}
                    >
          ...
          <aside className="m6-tier-1 min-h-0 rounded-md p-4">
            ...
            {selected ? (
              <div className="space-y-2 text-[11px]">
                <div className="rounded border border-dos-border bg-dos-panel/45 px-2 py-1.5 text-text/92">{selected.title}</div>
                ...
                <div className="flex flex-wrap gap-1.5 text-[10px] uppercase tracking-wide">
                  <button type="button" onClick={() => onAssign?.(selected.id)} className="rounded border border-dos-border px-2 py-1 text-textMuted hover:bg-dos-panel/52 hover:text-text/88">Assign</button>
                  <button type="button" onClick={() => onDefer?.(selected.id)} className="rounded border border-dos-border px-2 py-1 text-textMuted hover:bg-dos-panel/52 hover:text-text/88">Defer</button>
              ...
              <div className="rounded bg-dos-panel/35 px-2 py-2 text-[11px] text-textMuted/75">Select an intervention to inspect details.</div>
            )}
          </aside>
        </section>
          ...
          <div className="flex items-center justify-between gap-6">
            <div className="flex items-center gap-1.5">
              ...
              <div className="flex items-center gap-1 text-[10px] uppercase tracking-wide opacity-50">
                ...
                <button type="button" disabled={!selected} onClick={() => selected && onReject?.(selected.id)} className="px-2 py-1 hover:text-text hover:opacity-100 disabled:opacity-40 text-dos-sig-warning">Escalate</button>
