...
export const submitJob = async (job: RenderBridgeJob): Promise<{ externalJobId: string }> => {
  void job;
  const externalJobId = `runway-${Date.now()}-${Math.random().toString(16).slice(2, 7)}`;
  jobs.set(externalJobId, { startedAt: Date.now(), durationMs: 4600 });
...
export const getStatus = async (
  ...
  return { status: progress < 18 ? 'queued' : 'rendering', progress };
};

export const fetchResult = async (externalJobId: string): Promise<{ outputs: string[] }> => ({
  outputs: [`renders/${externalJobId}/preview.mp4`],
});
...
export const cancelJob = async (externalJobId: string): Promise<{ cancelled: boolean; message?: string }> => {
  ...
  return {
    cancelled: existed,
    message: existed ? 'Runway connector cancelled remote simulation.' : 'Runway connector did not find remote job during cancel.',
  };
};
