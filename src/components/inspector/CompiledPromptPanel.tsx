...
export const CompiledPromptPanel = ({ payload, currentJob }: CompiledPromptPanelProps) => {
  ...
  return (
    <div className="space-y-3">
      <div className="inline-flex gap-1 rounded-md border border-[var(--m6-border-soft)] bg-panel/22 p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.012)]">
        <button
      ...
      {payload ? (
        <div className="grid grid-cols-2 gap-2">
          <div className="rounded-md border border-[var(--m6-border-soft)] bg-panel/24 px-2.5 py-1.5">
            <div className="text-[8px] font-medium uppercase tracking-[0.08em] text-dos-text-muted/46">Active route</div>
            <div className="mt-0.5 break-all text-[9px] font-mono text-dos-text-muted/78">{payload.payload.routeContext.activeRoute}</div>
          </div>
          <div className="rounded-md border border-[var(--m6-border-soft)] bg-panel/24 px-2.5 py-1.5">
            <div className="text-[8px] font-medium uppercase tracking-[0.08em] text-dos-text-muted/46">Target engine</div>
            <div className="mt-0.5 text-[9px] font-mono text-dos-text-muted/78">{payload.payload.routeContext.targetEngine}</div>
          </div>
        </div>
      ) : null}

      <div className="max-h-64 overflow-auto rounded-md border border-[var(--m6-border-soft)] bg-panel/20 p-3 m6-scrollbar shadow-[inset_0_1px_0_rgba(255,255,255,0.012)]">
        {mode === 'prompt' ? (
          <div className="text-[12px] leading-relaxed font-light tracking-tight text-dos-text/78 [text-wrap:pretty]">
            {output}
          </div>
        ) : (
